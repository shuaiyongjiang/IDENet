...
def randomCrop(image, label, edge, depth):
    ...
    return image.crop(random_region), label.crop(random_region), edge.crop(random_region), depth.crop(random_region)
...
class SalObjDataset(data.Dataset):
    ...
    def filter_files(self):
        ...
        edges = []
        depths = []
        for img_path, gt_path, edge_path, depth_path in zip(self.images, self.gts, self.edges, self.depths):
    ...
    def resize(self, img, gt, edge, depth):
        ...
        if h < self.trainsize or w < self.trainsize:
            ...
            return img.resize((w, h), Image.BILINEAR), gt.resize((w, h), Image.NEAREST), \
                   edge.resize((w, h), Image.NEAREST), depth.resize((w, h),Image.NEAREST)
        else:
            return img, gt, edge, depth
...
def get_loader(image_root, gt_root, depth_root, edge_root, batchsize, trainsize, shuffle=True, num_workers=8, pin_memory=True):
